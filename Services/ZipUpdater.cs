...
/// <summary>
/// ZIP 版の自前更新(#12)。ダウンロード → 検証 → 展開まで。
/// XTimelineViewer の同名クラスの移植。
///
...
internal sealed class ZipUpdater
{
    ...
    /// <summary>動作中のプロセスに合う ZIP の名前を決める。</summary>
    ...
    internal static Asset? SelectAsset(string releaseJson, Architecture arch)
    {
        ...
        // .sha256 が無いリリース(v1.0.4 以前)は自前更新の対象にしない。
    ...
    internal static bool LooksLikeApp(string dir)
        => File.Exists(Path.Combine(dir, "Petapeta.exe"))
        && File.Exists(Path.Combine(dir, "Petapeta.dll"));
    ...
    /// <summary>ZIP を落として一時ファイルへ書く。進捗は 0.0〜1.0(長さ不明なら報告しない)。</summary>
        ...
        string url, string destPath, IProgress<double>? progress = null, CancellationToken ct = default)
    {
        ...
        req.Headers.TryAddWithoutValidation("User-Agent", "Petapeta");
    ...
    internal static bool CanWriteTo(string dir)
    {
        try
        {
            var probe = Path.Combine(dir, $".petapeta-write-probe-{Guid.NewGuid():N}");
        ...
        Asset asset, string installDir, IProgress<double>? progress = null, CancellationToken ct = default)
    {
        ...
        var work = Path.Combine(Path.GetTempPath(), "petapeta-update", Guid.NewGuid().ToString("N"));
        ...
        try
        {
            ...
            UpdateService.Trace($"ZipUpdater: 展開まで完了 {staging}");
            return staging;
        }
        finally
        {
            // ZIP は数十 MB ある。成功しても失敗しても残さない。
            try { if (Directory.Exists(work)) Directory.Delete(work, recursive: true); }
            catch (Exception ex) { UpdateService.Trace($"ZipUpdater: 一時 ZIP を消せませんでした {ex.Message}"); }
        }
    }
}
