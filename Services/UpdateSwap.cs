...
/// <summary>
/// インストール先の差し替え(#12)。XTimelineViewer の同名クラスの移植。
///
/// 実行中の exe / DLL は上書きできないので、置き換えを行うプロセスは
/// <b>置き換え対象の外</b>にいる必要がある。専用の更新用 exe を足す案もあるが、
/// 「無署名の小さな実行ファイルがネットから落として展開して自分を置き換える」は
/// Defender に疑われる振る舞いそのもの。バイナリもスクリプトも増やさず、
...
internal static class UpdateSwap
{
    ...
    // 展開直後のファイルをウイルス対策が走査している間は改名が通らない。
        ...
        string installDir, string stagingDir, int attempts = DefaultAttempts, int delayMs = DefaultDelayMs)
    {
        ...
        try
        {
            ...
            // ここは移動ではなくコピー。この処理を行っているプロセス自身が
            // 展開先から動いているため、展開先は自分の exe と DLL に掴まれていて
            // 動かせない。読み取りはできるので、コピーなら通る。
            ...
            UpdateService.Trace($"UpdateSwap: 差し替え完了 {installDir}");
            return SwapResult.Succeeded;
        }
        catch (Exception ex)
        {
            UpdateService.Trace($"UpdateSwap: 失敗 {ex}");
            ...
            try
            {
                ...
                UpdateService.Trace("UpdateSwap: 旧版へ戻した");
                return SwapResult.RolledBack;
            }
            catch (Exception restoreEx)
            {
                // ここまで来ると自動では直せない。ログに残して人手に委ねる。
                UpdateService.Trace($"UpdateSwap(rollback): 失敗 {restoreEx}");
    ...
    /// <summary>
    /// ディレクトリの中身を丸ごとコピーする。動いているプロセスの居場所を
    /// 移動できない以上ほかに手が無い。数秒で終わる。
    /// </summary>
    ...
    private static void CopyFileWithRetry(string from, string to, int attempts, int delayMs)
    {
        for (var i = 1; ; i++)
        {
            ...
            {
                if (i == 1) UpdateService.Trace($"UpdateSwap: {from} が掴まれている。離すのを待つ");
    ...
    /// 展開直後のフォルダーはウイルス対策のスキャンや検索インデックスに
    /// 掴まれていて Move が転ぶことがある。いずれも数秒で離すので、
        ...
        string source, string dest, int attempts = DefaultAttempts, int delayMs = DefaultDelayMs)
    {
        ...
        for (var i = 1; ; i++)
        {
            try
            {
                ...
                if (i > 1) UpdateService.Trace($"UpdateSwap: {i} 回目で改名できた {source}");
                return;
            }
            catch (Exception ex) when (i < attempts && IsTransient(ex))
            {
                if (i == 1) UpdateService.Trace($"UpdateSwap: {source} が掴まれている。離すのを待つ: {ex.Message}");
                Thread.Sleep(delayMs);
            }
        }
    }
    ...
    internal static void CleanupBackup(string installDir)
    {
        ...
        foreach (var dir in new[] { ZipUpdater.BackupDirFor(installDir), ZipUpdater.StagingDirFor(installDir) })
        {
            ...
            if (DeleteBestEffort(dir)) UpdateService.Trace($"UpdateSwap: 掃除した {dir}");
        }
    }
    ...
    private static bool DeleteBestEffort(string dir)
    {
        ...
        catch (Exception ex)
        {
            ...
            UpdateService.Trace($"UpdateSwap: {dir} を消せませんでした(次回また試す): {ex.Message}");
            return false;
        }
    }

    /// <summary>差し替え役へ渡す引数。インストール先と、終了を待つべきプロセス ID。</summary>
    internal static string[] BuildFinishArgs(string installDir, int waitForPid)
        => [FinishArg, installDir, waitForPid.ToString()];

    /// <summary><see cref="BuildFinishArgs"/> の逆。形が合わなければ null。</summary>
        ...
        int pid, TimeSpan timeout, CancellationToken ct = default)
    {
        ...
        using (proc)
        {
            ...
            {
                UpdateService.Trace($"UpdateSwap: PID {pid} の終了を待ちきれませんでした");
                return false;
            }
        }
    }
