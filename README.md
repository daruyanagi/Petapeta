...
また、既定で動作ログ(処理したファイル名などを含む)をデータフォルダー配下の `Logs` に保存します。不要な場合は設定で「ログをファイルに保存」をオフにしてください。古いログは保持日数の設定に従って自動削除されます。

「画像 URL をダウンロードしてファイル化」(既定オン)が有効な場合、コピーしたテキストが画像の URL と判断されると、エクスプローラーへ切り替えたタイミングで**その URL への外部通信(ダウンロード)が発生**します。ループバック・プライベートアドレス宛は拒否されますが、外部への通信自体が望ましくない場合は設定でオフにしてください。
